...
/* prototype MCU independent IrDA implementation.
 * Many MCUs, inkl. MSP430, have USART IrDA encoders which could be used.
 * gpio_set and gpio_clear introduce delays which mess up the timings,
 * so we reverted to defines.
 */
static inline void _send_high(unsigned int count)
{
    ...
    for (unsigned int i = 0; i < carrierCount; i++) {
        ...
        BOOSTIR_TX_OUT |= BOOSTIR_TX_OUT_PIN;
        __delay_cycles(52);
...
static inline void _send_low(unsigned int count, gpio_t port)
{
    (void)port;
